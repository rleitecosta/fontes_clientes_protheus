...
If ntipo <> 1
	...
	//Novo modelo de fontes para impressao - Leandro - ITUP   (Courier New ou Tahoma ou arial) 
...
If lMv_ItDesc
	...
	For nX := 1 To nLenDet
		...
		//Aadd(aIndImp, oDanfe:nHorzRes() ) 
	    //Aadd(aIndImp, oReport:ThinLine() )    //leandro 
	Next
EndIf
...
For nY := 1 To nLenItens
	...
	//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
	//³Leandro - ITUP         
	//Separação dos itens do pedido - Inicio                                   ³
	nLinha :=nLinha + 03
		/*
		If nY < nLenItens
			If MV_PAR05=1 .And. nFolhas>1
				oDanfe:Say(875,497,"CONTINUA NO VERSO")
			Else  
		    	//oDanfe:Say(nLinha, 0 , Replicate("-", 265 ), oFont08:oFont)
		    	oDanfe:Line( nLinha,0,nLinha,602 )
		    	nLinha :=nLinha + 07
		    Endif 
		Endif 
		*/ 
	  oDanfe:Line( nLinha,0,nLinha,602 )
	  nLinha :=nLinha + 07
	//Separação dos itens do pedido - Fim                                      ³
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
Next nY
...
Local nFolhas		:= 0
Local nMaxDes	    := 54
Local nMaxI		:= 066 //MAXIMO DE ITENS PRIMEIRA PAGINA   //leandro 
