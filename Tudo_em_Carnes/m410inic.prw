...
Local _nX
Local _nItem
Local _cItem := "" // [Suzanna Paizan]

Local _nTamFil   := len( FWCodFil())
Local _cFilExec	 := left(alltrim(Upper(SuperGetMv("FS_TCEMPTR",," "))),_nTamFil)
...
// Se for Filial de transferЙncia de produtos, trazer somente os grupos desta Filial
if !empty(_cFilExec) .and. _cFilExec == alltrim(Upper(FWCodFil())) 

	_cGrupos := alltrim(Upper(SuperGetMv("FS_TCGRPRD",,"")))

	if !empty(_cGrupos)

		_cQuery += " and B1_GRUPO IN  " + FormatIn(_cGrupos,"|") + " " 
...
For _nX:=1 to len(_aProdutos)
	...
	Else
		...
		aCols[_nX][1] := _cItem//strzero(_nX,tamsx3("C6_ITEM")[1] ) // => [Suzanna Paizan] Alterado para corrigir a numeracao acima de 99, que estava sendo gerada numerica e nao estava sendo gravada corretamente pois o campo e caracter 2.
		aCols[_nX][Ascan(aHeader,{|x|AllTrim(x[2])=="C6_PRODUTO"})] := _aProdutos[_nX][2]
		aCols[_nX][Ascan(aHeader,{|x|AllTrim(x[2])=="C6_DESCRI"})]  := _aProdutos[_nX][3]
		aCols[_nX][Ascan(aHeader,{|x|AllTrim(x[2])=="C6_UM"})]      := _aProdutos[_nX][4]
