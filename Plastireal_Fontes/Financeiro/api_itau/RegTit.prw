...
user function RegTit(cFil,cCliente,cLoja,cPrefixo,cNum,cParcela,cTipo) //parùmetros de SE1
    ...
    if SE1->E1_XSTITAU == "1"           //Caso jù registrado
        return .T.
    endif
    ...
    if SE1->(DBSeek(cFil+cCliente+cLoja+cPrefixo+cNum+cParcela+cTipo))
        ...
        if aT('OK',aResConect[1]) > 0
            ...
            if aT('OK',cHeadRet) > 0
                //Criar um campo E1_XSTITAU para armazenar se tùtulo estù na cobranùa
                RecLock("SE1",.F.)
                SE1->E1_XSTITAU := "1"                                                          //Grava situaùùo do boleto (0=nùo registrado, 1=registrado)
                SE1->E1_PORTADO := "341"
                SE1->E1_AGEDEP := "5607"
                SE1->E1_CONTA := "01575"
                MsUnlock()
                ...
                ConOut( "Nùo foi possùvel conectar com a API de registro de cobranùa: "+cHeadRet+" "+sPostRet)
            endif

        else
            ConOut( "Nùo foi possùvel conectar com o Autorizador: "+aResConect[1]+" "+aResConect[2])
        endif
    endif
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------
// Funùùo que busca o token da API
//------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------
// Funùùo que gera o JSON reduzido da API
//------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
Static Function GeraJSON1(cFil,cCliente,cLoja,cPrefixo,cNum,cParcela,cTipo) //fil 0101 cliente 19381564 loja 0001 pref FAT Num 13436 Tipo NF
    ...
    Local cCodigo
    Local cLojaCli
    Local cAgencia      := SUPERGETMV("FS_AGENBI",.F.)                              //Agùncia formato 9999                      "5607"
    ...
    Local cEspecie      := SUPERGETMV("FS_ESPITAU",.F.)                             //Espùcie                                   "01" 

    Local cTPag         := SUPERGETMV("FS_TPITAU",.F.)                              //FS_TPITAU                                 3=Com data de vencimento
    Local cIPP          := SUPERGETMV("FS_IPPITAU",.F.)                             //FS_IPPITAU                                true=aceita false=nùo aceita
    ...
    cJson += '{'+CRLF
    cJson += '    "tipo_ambiente": 2,'+CRLF                                                                     //1=teste 2=produùùo Fixo
    ...
    cJson += '                    },'+CRLF                      
    cJson += '    "titulo_aceite": "S",'+CRLF                                                                   //S=Cobranùa N=Proposta
    cJson += '    "pagador":      {'
    if SA1->A1_PESSOA == 'F'                        
    ...
    cJson += '                    },'+CRLF 
    cJson += '    "nosso_numero": "' +cNosNu8P+ '",'+CRLF                                                       //Nosso nùmero = SE1_E1NUMBCO
    ...
    cJson += '    "seu_numero": "'+SubStr(alltrim(cNum+cParcela),1,10)+'",' +CRLF                                                //nùmero do tùtulo
    cJson += '    "especie": "' +cEspecie+ '",'+CRLF                                                            //Algumas opùùes
    ...
    cJson += '    "indicador_pagamento_parcial": "' +cIPP+ '",' +CRLF                                           //FS_IPPITAU                                        //true=aceita false=nùo aceita
    cJson += '    "juros":        {'+CRLF                       
    ...
    cJson += '                                "tipo_autorizacao_recebimento": "' +cTAR+ '"'+CRLF                //FS_TARITAU                      //vùrios tipos parùmetro?
    cJson += '                                }'+CRLF
    cJson += '}'

    MemoWrite("\system\json_api_itau\json_api_itau.txt",cJson)

    RestArea(aArea)
Return cJson

//------------------------------------------------------------------------------------------------------------------------------------------------------------------
// Funùùo que gera o JSON completo da API
//------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
Static Function GeraJSON2()
    ...
    cJson += '     {'+CRLF
    cJson += '        "tipo_ambiente": 1,' +CRLF                                           //1=teste 2=produùùo
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------
// Funùùo que gera o dùgito verificador do nosso nùmero
//------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
for i=len(cString) to 1 step -1                                 //constroi o vetor de multiplicaùùo do valor pelos pesos
...
for k=1 to len(cRes)                                            //soma o os dùgitos da string                                     
