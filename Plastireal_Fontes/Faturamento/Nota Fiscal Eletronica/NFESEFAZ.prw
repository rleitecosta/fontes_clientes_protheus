...
#DEFINE MAXMENLIN  (100-1) // Máximo de caracteres por linha de dados adicionais
...
private cSF2Filial:=xFilial("SF2")
private cSF3Filial:=xFilial("SF3")
private cSF4Filial:=xFilial("SF4")
private cSFTFilial:=xFilial("SFT")
private cSD2Filial:=xFilial("SD2")
private cSB1Filial:=xFilial("SB1")
private cSA1Filial:=xFilial("SA1")
private cCD2Filial:=xFilial("CD2")
private cSC6Filial:=xFilial("SC6")
private cSC5Filial:=xFilial("SC5")
private cSA2Filial:=xFilial("SA2")
private cSE1Filial:=xFilial("SE1")
private cCD9Filial:=xFilial("CD9")
private cSA4Filial:=xFilial("SA4")
private cDA3Filial:=xFilial("DA3")
private cSL1Filial:=xFilial("SL1")
private cSYAFilial:=xFilial("SYA")
private cCE3Filial:=xFilial("CE3")
private cSD1Filial:=xFilial("SD1")
private cSF1Filial:=xFilial("SF1")
private cSB5Filial:=xFilial("SB5")
private cDY3Filial:=xFilial("DY3")
private cCD6Filial:=xFilial("CD6")
private cCD7Filial:=xFilial("CD7")
private cCD8Filial:=xFilial("CD8")
private cCDRFilial:=xFilial("CDR")
private cCDSFilial:=xFilial("CDS")
private cF0AFilial:=xFilial("F0A")
private cSA7Filial:=xFilial("SA7")
private cSA5Filial:=xFilial("SA5")
private cSB8Filial:=xFilial("SB8")
private cCDVFilial:=xFilial("CDV")
private cCDYFilial:=xFilial("CDY")
private cCDAFilial:=xFilial("CDA")
private cD3EFilial:=xFilial("D3E")
private cCDLFilial:=xFilial("CDL")
private cCDNFilial:=xFilial("CDN")
private cF0LFilial:=xFilial("F0L")
private cSE2Filial:=xFilial("SE2")
private cSF8Filial:=xFilial("SF8")
private cSC7Filial:=xFilial("SC7")
private cCD5Filial:=xFilial("CD5")
private cSF7Filial:=xFilial("SF7")
private cSE4Filial:=xFilial("SE4")
private cSL4Filial:=xFilial("SL4")
private cSBZFilial:=xFilial("SBZ")
...
If cTipo == "1"
    ...
    If FindFunction("LjR30Sped")
        ...
        dbSelectArea("SF2")
        dbSetOrder(1)
        If MsSeek(cSF2Filial+cNota+cSerie+cClieFor+cLoja)
    ...
    For nCountNF:=1 To Len(aCupRefLoj)
        ...
        If MsSeek(cSF2Filial+cNota+cSerie+cClieFor+cLoja)
            ...
            dbSelectArea("SF3")
            dbSetOrder(4)
            If MsSeek(cSF3Filial+cClieFor+cLoja+cNota+cSerie)
                While !SF3->(Eof()) .And. cClieFor+cLoja+cNota+cSerie == SF3->F3_CLIEFOR+SF3->F3_LOJA+SF3->F3_NFISCAL+SF3->F3_SERIE
                    ...
                    MsSeek(cSFTFilial+"S"+SF3->F3_CLIEFOR+SF3->F3_LOJA+SF3->F3_SERIE+SF3->F3_NFISCAL+SF3->F3_IDENTFT)

                    dbSelectArea("SD2")
                    dbSetOrder(3)
                    MsSeek(cSD2Filial+SFT->FT_NFISCAL+SFT->FT_SERIE+SFT->FT_CLIEFOR+SFT->FT_LOJA+SFT->FT_PRODUTO)
                    dbSelectArea("SF4")
                    dbSetOrder(1)
                    MsSeek(cSF4Filial+SD2->D2_TES)
                    If SF3->F3_TIPO =="S"
                        If SF3->F3_RECISS =="1"
                            cSitTrib:="R"
                    ...
                    dbSelectArea("SD2")
                    dbSetOrder(3)
                    MsSeek(cSD2Filial+SF2->F2_DOC+SF2->F2_SERIE+SF2->F2_CLIENTE+SF2->F2_LOJA)

                    dbSelectArea("SA1")
                    dbSetOrder(1)
                    MsSeek(cSA1Filial+SF2->F2_CLIENTE+SF2->F2_LOJA)

                    cTpPessoa:=SA1->A1_TPESSOA

                    If nCont == 1
                        Do While !SD2->(Eof ()) .And. cSD2Filial == (cAliasSD2)->D2_FILIAL .And.;
                        ...
                        dbSetOrder(3)
                           ...
                           dbSelectArea("CD2")
                        dbSetOrder(1)
                        If DbSeek(cCD2Filial+"S"+SF2->F2_SERIE+SF2->F2_DOC+cClieFor+cLoja+PadR(SD2->D2_ITEM,4)+(cAliasSD2)->D2_COD)
            ...
            ElseIf FunName() == "SPEDNFSE"
                ...
                If !Upper(SA1->A1_EST) == "EX"
                    SC6->(dbSetOrder(4))
                    SC5->(dbSetOrder(1))
                    If (SC6->(MsSeek(cSC6Filial+SF2->F2_DOC+SF2->F2_SERIE)))
                        SC5->(MsSeek(cSC5Filial+SC6->C6_NUM))
                        ...
                        Else
                            SA2->(dbSetOrder(1))
                            SA2->(MsSeek(cSA2Filial+SC5->C5_FORNISS+"00"))
                ...
                dbSelectArea("SF3")
                dbSetOrder(4)
                MsSeek(cSF3Filial+SF2->F2_CLIENTE+SF2->F2_LOJA+SF2->F2_DOC+SF2->F2_SERIE)

                While !Eof() .And. cSF3Filial == SF3->F3_FILIAL .And.;
                    ...
                    dbSelectArea("SE1")
                    SE1->(dbSetOrder(2))
                    If SE1->(dbSeek(cSE1Filial+SF3->F3_CLIEFOR+SF3->F3_LOJA+SF3->F3_SERIE+SF3->F3_NFISCAL))
                        While !SE1->(Eof()) .And. cSE1Filial == SE1->E1_FILIAL .And.;
                    ...
                    If len(aSX560) > 0
                        ...
                        If cDescServ$"1"
                            SC6->(dbSetOrder(4))
                            SC5->(dbSetOrder(1))
                            MsSeek(cSC6Filial+SF3->F3_NFISCAL+SF3->F3_SERIE)
                            MsSeek(cSC5Filial+SC6->C6_NUM)
            ...
            Else
                ...
                //ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
                  ...
                  If ("CF" $ SF2->F2_ESPECIE .OR. "NFCE" $ SF2->F2_ESPECIE .OR. "SATCE" $ SF2->F2_ESPECIE .OR. (LjAnalisaLeg(18)[1] .AND. "ECF" $ SF2->F2_ESPECIE .AND. ("S" $ SF2->F2_ECF) )) .AND. !Empty(SF2->F2_NFCUPOM)
                    ...
                    If DbSeek( cSF2Filial + cNumNfCup + cSerNfCup)
                ...
                If !SF2->F2_TIPO $ "DB"
                    ...
                    If (SF2->(ColumnPos("F2_CLIRET")) > 0 .And. SF2->(ColumnPos("F2_LOJARET")) > 0) .And. !Empty(SF2->F2_CLIRET+SF2->F2_LOJARET) .And. SF2->F2_CLIRET+SF2->F2_LOJARET<>SF2->F2_CLIENTE+SF2->F2_LOJA
                        dbSelectArea("SA1")
                        dbSetOrder(1)
                        If MsSeek(cSA1Filial+SF2->F2_CLIRET+SF2->F2_LOJARET)
                            ...
                            aadd(aRetirada,IIF(Empty(SA1->A1_PAIS),"1058"  ,Posicione("SYA",1,cSYAFilial+SA1->A1_PAIS,"YA_SISEXP")))
                            aadd(aRetirada,IIF(Empty(SA1->A1_PAIS),"BRASIL",Posicione("SYA",1,cSYAFilial+SA1->A1_PAIS,"YA_DESCR" )))
                    ...
                    If SF2->(FieldPos("F2_CLIENT"))<>0 .And. !Empty(SF2->F2_CLIENT+SF2->F2_LOJENT) .And. (SF2->F2_CLIENT+SF2->F2_LOJENT<>SF2->F2_CLIENTE+SF2->F2_LOJA .OR. cTpOper == "1" )
                        ...
                        dbSelectArea("SA1")
                        dbSetOrder(1)
                        If MsSeek(cSA1Filial+SF2->F2_CLIENT+SF2->F2_LOJENT)
                            ...
                            aadd(aEntrega,IIF(Empty(SA1->A1_PAIS),"1058"  ,Posicione("SYA",1,cSYAFilial+SA1->A1_PAIS,"YA_SISEXP")))
                            aadd(aEntrega,IIF(Empty(SA1->A1_PAIS),"BRASIL",Posicione("SYA",1,cSYAFilial+SA1->A1_PAIS,"YA_DESCR" )))
                    ...
                    dbSelectArea("SA1")
                    dbSetOrder(1)
                    SA1->(MsSeek(cSA1Filial+SF2->F2_CLIENTE+SF2->F2_LOJA))
                    ...
                    */
                    If !lUsaCliEnt
                        ...
                        If !Empty(AllTrim(SF2->F2_CLIENT)) .And. !Empty(AllTrim(SF2->F2_LOJENT))
                            If Len(aEntrega) > 0
                                ...
                                If aEntrega[08] <> IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) .And. !lCNPJIgual //aEntrega[08] <> Upper(SA1->A1_EST)
                                    SA1->(MsSeek(cSA1Filial+SF2->F2_CLIENT+SF2->F2_LOJENT))
                                EndIf
                        ...
                        Else
                            If !Empty(cCliNota+cCliLoja)
                                SA1->(MsSeek(cSA1Filial+cCliNota+cCliLoja))   //Busca os dados do cliente da Nota sobre Cupom para montar os dados do destinatário do XML
                            Else
                                SA1->(MsSeek(cSA1Filial+SF2->F2_CLIENTE+SF2->F2_LOJA))
                            EndIf
                        EndIf
                    ...
                    If !Empty(SA1->A1_MENSAGE) .and. SA1->(Formula(A1_MENSAGE)) <> Nil .and. !EMPTY(SA1->(Formula(A1_MENSAGE)))
                        If cMVNFEMSA1=="C"
                            cMensCli:=MsgMaxLin(SA1->(Formula(A1_MENSAGE)))
                        ElseIf cMVNFEMSA1=="F"
                            cMensFis:=MsgMaxLin(SA1->(Formula(A1_MENSAGE)))
                        EndIf
                    EndIf
                    ...
                    aadd(aDest,Upper(SA1->A1_EST))
                    aadd(aDest,SA1->A1_CEP)
                    aadd(aDest,IIF(Empty(SA1->A1_PAIS),"1058"  ,Posicione("SYA",1,cSYAFilial+SA1->A1_PAIS,"YA_SISEXP")))
                    aadd(aDest,IIF(Empty(SA1->A1_PAIS),"BRASIL",Posicione("SYA",1,cSYAFilial+SA1->A1_PAIS,"YA_DESCR" )))
                    aadd(aDest,AllTrim(SA1->A1_DDD)+SA1->A1_TEL)
                    If !Upper(SA1->A1_EST) == "EX"
                ...
                Else
                    ...
                    If !Empty(AllTrim(SF2->F2_CLIENT)) .And. !Empty(AllTrim(SF2->F2_LOJENT))
                        MsSeek(cSA2Filial+SF2->F2_CLIENT+SF2->F2_LOJENT)
                    Else
                        MsSeek(cSA2Filial+SF2->F2_CLIENTE+SF2->F2_LOJA)
                    EndIf
                    aDest:={}
                    ...
                    aadd(aDest,IIF(Empty(SA2->A2_PAIS),"1058"  ,Posicione("SYA",1,cSYAFilial+SA2->A2_PAIS,"YA_SISEXP")))
                    aadd(aDest,IIF(Empty(SA2->A2_PAIS),"BRASIL",Posicione("SYA",1,cSYAFilial+SA2->A2_PAIS,"YA_DESCR")))
                ...
                If !Empty(SF2->F2_TRANSP)
                    dbSelectArea("SA4")
                    dbSetOrder(1)
                    MsSeek(cSA4Filial+SF2->F2_TRANSP)
                    ...
                    If !Empty(SF2->F2_VEICUL1)
                        dbSelectArea("DA3")
                        dbSetOrder(1)
                        MsSeek(cDA3Filial+SF2->F2_VEICUL1)
                        ...
                        If !Empty(SF2->F2_VEICUL2)
                            ...
                            dbSelectArea("DA3")
                            dbSetOrder(1)
                            MsSeek(cDA3Filial+SF2->F2_VEICUL2)
                            ...
                            If !Empty(SF2->F2_VEICUL3)
                                ...
                                dbSelectArea("DA3")
                                dbSetOrder(1)
                                MsSeek(cDA3Filial+SF2->F2_VEICUL3)
                    ...
                    ElseIf lNfCup
                        SL1->(dbSetOrder(2))
                        SL1->(MsSeek(cSL1Filial+SF2->F2_SERIE+SF2->F2_DOC))
                ...
                If GetNewPar("MV_SUFRAMA",.F.) .And. !empty(aDest[15])
                    cMensFis += MsgMaxLin("Código Suframa: "+alltrim(aDest[15])+".")
                Endif
                ...
                cChave:=""
                If !lNfCup
                    cChave:=cSF3Filial+SF2->F2_CLIENTE+SF2->F2_LOJA+SF2->F2_DOC+SF2->F2_SERIE
                Else
                    cChave:=cSF3Filial+cCliNota+cCliLoja+cNumNfCup+cSerNfCup
                Endif
                ...
                If MsSeek(cChave)
                    ...
                    If lSimpNac
                        ...
                        cChave:=cSD2Filial + SFT->FT_NFISCAL + SFT->FT_SERIE + SFT->FT_CLIEFOR + SFT->FT_LOJA
                        ...
                        If SF2->F2_TIPO == "D"
                            cMensTmp:="Documento emitido por ME ou EPP optante pelo Simples Nacional. "
                            cMensTmp+="Base de cálculo do ICMS: R$ " + Str(SF2->F2_BASEICM, 14, 2) + ". "
                            cMensTmp+="Valor do ICMS: R$ " + Str(SF2->F2_VALICM, 14, 2) + ". "
                        Else
                            If SF2->F2_VALICM > 0 .And. nValSimprem > 0   // Novo Tratamento
                                cMensTmp:="Documento emitido por ME ou EPP optante pelo Simples Nacional."
                                cMensTmp+="Permite o aproveitamento do credito de ICMS no valor de R$ " + IIf( Empty(nValSimprem),Str(SF2->F2_VALICM, 14, 2), Str(nValSimprem, 14, 2) ) + " corresponde a aliquota de "+str(SD2->D2_PICM,5,2)+ "% , nos termos do art. 23 da LC 123/2006."
                            Else
                                cMensTmp:="Documento emitido por ME ou EPP optante pelo Simples Nacional. Nao gera direito a credito fiscal de IPI."
                            EndIf
                        EndIf
                        cMensFis+=MsgMaxLin(cMensTmp)
                    EndIf
                EndIf
                dbSelectArea("SF2")
                //ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
                ...
                If !Empty(SF2->F2_DUPL)
                    ...
                    #ELSE
                        MsSeek(cSE1Filial+SF2->F2_PREFIXO+SF2->F2_DOC)
                    #ENDIF
                    While !Eof() .And. cSE1Filial == (cAliasSE1)->E1_FILIAL .And.;
                ...
                If lNfCup .And. SM0->M0_ESTCOB $ "CE/DF"
                    ...
                    DbSelectArea( "SF2" )
                    DbSetOrder(1)  // F2_DOC + F2_SERIE + F2_CLIENTE + F2_LOJA
                    DbSeek( cSF2Filial + cNumNfCup + cSerNfCup)
                EndIf
                ...
                If !Empty(cIdRecopi)
                    If AliasIndic("CE3")
                        CE3->(DbSetOrder(1))
                        If CE3->(DbSeek(cCE3Filial+Alltrim(cIdRecopi)))
                ...
                nCountIT:=0
                While !Eof() .And. cSD2Filial == (cAliasSD2)->D2_FILIAL .And.;
                    ...
                    If lGE .and. lNfCup
                        ...
                        If DbSeek(cSB1Filial+(cAliasSD2)->D2_COD)
                    ...
                    If lContinua
                        ...
                        If lNfCup
                            ...
                            If Dbseek(cSD2Filial+SF2->F2_DOC+SF2->F2_SERIE+SF2->F2_CLIENTE+SF2->F2_LOJA)
                                cD2Tes:=SD2->D2_TES
                            EndIf

                            If DbSeek(cSD2Filial+cNumNfCup+cSerNfCup+cCliNota+cCliLoja+(cAliasSD2)->D2_COD+cNumitem)
                                cD2Cfop:=SD2->D2_CF
              ...
              // Posiciono na TES do cupom fiscal para pegar a natureza de operação da nf sobre cupom
                        ...
                        If !Empty((cAliasSD2)->D2_NFORI)
                            If (cAliasSD2)->D2_TIPO $ "DBN"
                                dbSelectArea("SD1")
                                dbSetOrder(1)
                                If ( MsSeek(cSD1Filial+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+PADL(alltrim((cAliasSD2)->D2_ITEMORI),TamSx3("D2_ITEMORI")[1],"0")) ) .OR. ;
                                    ( MsSeek(cSD1Filial+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA) .And. Empty(cMVREFNFE) )
                                    ...
                                    If !Empty((cAliasSD2)->D2_IDENTB6)
                                        ...
                                        dbSelectArea("SD1")
                                        dbSetOrder(4)
                                        If MsSeek(cSD1Filial+(cAliasSD2)->D2_IDENTB6)
                                            dbSetOrder(1)
                                        Else
                                            dbSetOrder(1)
                                            SD1->(DbGoTo(nSD1Pos))
                                    ...
                                    dbSelectArea("SF1")
                                    dbSetOrder(1)
                                    MsSeek(cSF1Filial+SD1->D1_DOC+SD1->D1_SERIE+SD1->D1_FORNECE+SD1->D1_LOJA+SD1->D1_TIPO)
                                    If SD1->D1_TIPO $ "DB"
                                        dbSelectArea("SA1")
                                        dbSetOrder(1)
                                        MsSeek(cSF1Filial+SD1->D1_FORNECE+SD1->D1_LOJA)
                                    Else
                                        dbSelectArea("SA2")
                                        dbSetOrder(1)
                                        MsSeek(cSA2Filial+SD1->D1_FORNECE+SD1->D1_LOJA)
                                    EndIf
                                ...
                                ElseIf (cAliasSD2)->D2_TIPO == "N"
                                    dbSelectArea("SFT")
                                       dbSetOrder(4)
                                       If MsSeek(cSFTFilial+"S"+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_NFORI)
                                        If SFT->FT_ESTADO == "EX" .or. ((SubStr(SM0->M0_CODMUN,1,2) == "35" .Or. SubStr(SM0->M0_CODMUN,1,2) == "29") .and. "REMESSA POR CONTA E ORDEM DE TERCEIROS" $ Upper(cNatOper) .and. lOrgaoPub )//(Venda para orgao publico - SP/BA/CFOP Remessa por conta e ordem de terceiros (cfop 5923/6923)- ch:TIDWCY
                                            ...
                                            dbSelectArea("SF3")
                                               dbSetOrder(4)
                                               MsSeek(cSF3Filial+SFT->FT_CLIEFOR+SFT->FT_LOJA+SFT->FT_NFISCAL+SFT->FT_SERIE)
                                        ...
                                        ElseIf Alltrim(SFT->FT_CFOP) $ cMVREFNFE
                                            ...
                                            dbSelectArea("SF3")
                                               dbSetOrder(4)
                                               If (MsSeek(cSF3Filial+SFT->FT_CLIEFOR+SFT->FT_LOJA+SFT->FT_NFISCAL+SFT->FT_SERIE))
                                                   If cChave <> dToS( SF3->F3_EMISSAO ) + SF3->F3_SERIE + SF3->F3_NFISCAL + SA1->A1_CGC + SM0->M0_ESTCOB + SF3->F3_ESPECIE + SF3->F3_CHVNFE;
                                                    .or. ( cAliasSD2 )->D2_ITEM <> cItemOr
        ...
        //                        If MsSeek(cSD2Filial+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
                                If MsSeek(cSD2Filial+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI)//+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
                                    dbSelectArea("SF2")
                                    dbSetOrder(1)
                                    MsSeek(cSF2Filial+SD2->D2_DOC+SD2->D2_SERIE+SD2->D2_CLIENTE+SD2->D2_LOJA)
                                    If !SD2->D2_TIPO $ "DB"
                                        dbSelectArea("SA1")
                                        dbSetOrder(1)
                                        MsSeek(cSA1Filial+SD2->D2_CLIENTE+SD2->D2_LOJA)
                                    Else
                                        dbSelectArea("SA2")
                                        dbSetOrder(1)
                                        MsSeek(cSA2Filial+SD2->D2_CLIENTE+SD2->D2_LOJA)
                                        lComplDev:=.T.
                                    EndIf
                                    //ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
                                    //³Obtem os dados de nota fiscal de produtor rural referenciada                                  ³
                        ...
                        dbSelectArea("SB1")
                        dbSetOrder(1)
                        MsSeek(cSB1Filial+(cAliasSD2)->D2_COD)

	                    if (!empty(SB1->B1_XMENS))
	                    	cUsrMsgCli:="["+(cAliasSD2)->D2_COD+" :: "+allTrim(SB1->B1_XMENS)+"]"
	                    	aAdd(aUsrMsgCli,MsgMaxLin(cUsrMsgCli))
	                    endif

                        dbSelectArea("SB5")
                        dbSetOrder(1)
                        If MsSeek(cSB5Filial+(cAliasSD2)->D2_COD)
                        ...
                        dbSelectArea("DY3")
                           dbSetOrder(1)
                           If MsSeek(cDY3Filial+ (cAliasSB5)->B5_ONU)
                        ...
                        If lC6_CODINF .And. cInfAdPr <> "2" .And. !Empty(cInfAdPr)
                            SC6->(dbSetOrder(2))
                            If SC6->(MsSeek(cSC6Filial+(cAliasSD2)->(D2_COD+D2_PEDIDO+D2_ITEMPV)))
                                cUsrMsgCli:="[PEDIDO DE COMPRA: "+if(empty(SC6->C6_XCLIPED),space(len(SC6->C6_XCLIPED)),SC6->C6_XCLIPED)
                                cUsrMsgCli+=" :: PEDIDO DE VENDA: "+SC6->C6_NUM+"]"
                                aAdd(aUsrMsgCli,MsgMaxLin(cUsrMsgCli))
                                cInfAdPed:=Alltrim(MSMM(SC6->C6_CODINF,80))
                                if (empty(cInfAdPed))
                                    cInfAdPed:=allTrim(SC6->C6_VDOBS)
                                endif
                                If !Empty(cInfAdPed)
                        ...
                        If AliasIndic("CD9")
                            dbSelectArea("CD9")
                            dbSetOrder(1)
                            MsSeek(cCD9Filial + cChaveD2 )
                        EndIf
                        //Combustivel
                        If AliasIndic("CD6")
                            dbSelectArea("CD6")
                            dbSetOrder(1)
                            MsSeek(cCD6Filial+"S"+(cAliasSD2)->D2_SERIE+(cAliasSD2)->D2_DOC+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+Padr((cAliasSD2)->D2_ITEM,4)+(cAliasSD2)->D2_COD)
                        EndIf
                        //Medicamentos
                        If AliasIndic("CD7")
                            dbSelectArea("CD7")
                            dbSetOrder(1)
                            MsSeek(cCD7Filial + cChaveD2 )
                        EndIf
                        // Armas de Fogo
                        If AliasIndic("CD8")
                            dbSelectArea("CD8")
                            dbSetOrder(1)
                            MsSeek(cCD8Filial + cChaveD2 )
                        EndIf
                        ...
                        If lAnfavea
                            dbSelectArea("CDR")
                            dbSetOrder(1)
                            DbSeek(cCDRFilial+"S"+(cAliasSD2)->D2_DOC+(cAliasSD2)->D2_SERIE+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA)

                            dbSelectArea("CDS")
                            dbSetOrder(1)
                            cItem:=PADR((cAliasSD2)->D2_ITEM,TAMSX3("CDS_ITEM")[1])
                            DbSeek(cCDSFilial+"S"+(cAliasSD2)->D2_SERIE+(cAliasSD2)->D2_DOC+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+cItem+(cAliasSD2)->D2_COD)
                        EndIf
                        ...
                        dbSelectArea("SC5")
                        dbSetOrder(1)
                        MsSeek(cSC5Filial+(cAliasSD2)->D2_PEDIDO)

                        dbSelectArea("SC6")
                        dbSetOrder(1)
                        MsSeek(cSC6Filial+(cAliasSD2)->D2_PEDIDO+(cAliasSD2)->D2_ITEMPV+(cAliasSD2)->D2_COD)
                        ...
                        If !AllTrim(SC5->C5_MENNOTA) $ cMensCli
                            ...
                            EndIf
                            If !Empty(cFieldMsg) .and. SC5->(FieldPos(cFieldMsg)) > 0 .and. !Empty(&("SC5->"+cFieldMsg))
                                cMensCli+=MsgMaxLin(alltrim(&("SC5->"+cFieldMsg)))
                            ElseIf !(IIF( SF2->(FieldPos("F2_MENNOTA")) > 0, AllTrim(SF2->F2_MENNOTA),AllTrim(SC5->C5_MENNOTA)) $ cMensCli)
                                cMensCli += MsgMaxLin(IIF( SF2->(FieldPos("F2_MENNOTA")) > 0, AllTrim(SF2->F2_MENNOTA),AllTrim(SC5->C5_MENNOTA)))
                            EndIf

                        EndIf
                        If !Empty(SC5->C5_MENPAD) .And. !AllTrim(FORMULA(SC5->C5_MENPAD)) $ cMensFis
                            cMensFis += MsgMaxLin(AllTrim(FORMULA(SC5->C5_MENPAD)))
                        EndIf
                        If !Empty( cNumNfCup )
                            ...
                            If Len(aItemCupRef) > 0
                                ...
                                if !upper(Alltrim(cMsgCup)) $ upper(Alltrim(cMensCli))
                                    ...
                                    cMensCli +=MsgMaxLin(" CF/SERIE: " + AllTrim((cAliasSD2)->D2_DOC) + " " + Alltrim((cAliasSD2)->D2_SERIE) +" ECF:" + Alltrim((cAliasSD2)->D2_PDV))
                                EndIf
                            Else
                                DbSelectArea("SFT")
                                DbSetOrder(1)
                                If SFT->(DbSeek((cSD2Filial+"S"+ cSerNfCup + cNumNfCup )))
                                        ...
                                        IF !Alltrim(SFT->FT_OBSERV) $ Alltrim(cMensCli)
                                            if upper( "F - simples faturamento" ) $  upper( Alltrim(SFT->FT_OBSERV) )
                                                ...
                                                If "DEVOLUCAO N.F." $ Upper(SFT->FT_OBSERV)
                                                    cMensCli +=MsgMaxLin(" " + StrTran(AllTrim(SFT->FT_OBSERV),"N.F.","C.F."))
                                                ElseIf !lNfCupNFCE .and. !lNfCupSAT
                                                    cMensCli +=MsgMaxLin(" " + AllTrim(SFT->FT_OBSERV))
                                                EndIf
                                            endif
                                        EndIf
                                       EndIf
...
*                                cMensCli += ' '
                            ...
                            if SM0->M0_ESTENT == "PR"
                                cMensCli += MsgMaxLin(" Nota fiscal emitida sem destaque do ICMS conforme artigo 9. do Anexo IX do RICMS-PR/2017.")
                            else
                                cMensCli += MsgMaxLin(" Nota fiscal emitida sem destaque do ICMS.")
                            endif
                        endif
                        ...
                        If Empty(aVeiculo)
                            DbSelectArea("DA3")
                            DbSetOrder(1)
                            If DbSeek(cDA3Filial+Iif(SC5->(FieldPos("C5_VEICULO")) > 0 ,SC5->C5_VEICULO,""))
                                aadd(aVeiculo,DA3->DA3_PLACA)
                                aadd(aVeiculo,DA3->DA3_ESTPLA)
                                aadd(aVeiculo,Iif(DA3->(FieldPos("DA3_RNTC")) > 0 ,DA3->DA3_RNTC,iif(!Empty(cAnttRntrc),cAnttRntrc,"")))//RNTC
                            EndIf
                        ...
                        If SF4->(ColumnPos("F4_FORINFC") ) > 0  .And. !Empty(SF4->F4_FORINFC) .And. Formula(SF4->F4_FORINFC) <> NIL .And. ( ( cMVNFEMSF4=="C" .And. !AllTrim(Formula(SF4->F4_FORINFC)) $ cMensCli ) .Or. (cMVNFEMSF4=="F" .And. !AllTrim(Formula(SF4->F4_FORINFC))$cMensFis) )
                            If cMVNFEMSF4=="C"
                                cMensCli    +=    MsgMaxLin(SF4->(Formula(F4_FORINFC)))
                            ElseIf cMVNFEMSF4=="F"
                                cMensFis    +=    MsgMaxLin(SF4->(Formula(F4_FORINFC)))
                            EndIf
                        ElseIf !Empty(SF4->F4_FORMULA) .And. Formula(SF4->F4_FORMULA) <> NIL .And. ( ( cMVNFEMSF4=="C" .And. !AllTrim(Formula(SF4->F4_FORMULA)) $ cMensCli ) .Or. (cMVNFEMSF4=="F" .And. !AllTrim(Formula(SF4->F4_FORMULA))$cMensFis) )
                            If cMVNFEMSF4=="C"
                                cMensCli    +=    MsgMaxLin(SF4->(Formula(F4_FORMULA)))
                            ElseIf cMVNFEMSF4=="F"
                                cMensFis    +=    MsgMaxLin(SF4->(Formula(F4_FORMULA)))
                            EndIf
                        EndIf
                        ...
                        If lSb1CT
                            If lMvImpFecp  .And. SB1->B1_X_CT$cMVAEHC
                                If (lValFecp .Or. lVfecpst)
                                    DbSelectArea("SFT")
                                    DbSetOrder(1)
                                    If SFT->(DbSeek((cSFTFilial + cChaveD2 )))
                                        If SFT->FT_VFECPST > 0
                                               cMensFis += MsgMaxLin(" Cod.Prod: " + Alltrim((cAliasSD2)->D2_COD) + IIF(SB1->B1_X_CT$cMVAEHC," AEHC ","") + " BC R$: " + Alltrim(Transform(SFT->FT_BASERET,"@E 999,999,999.99"))  + " o adicional de " + Alltrim(Str(SFT->FT_ALQFECP, 14, 2))+"%" + " valor FECP R$ " + Alltrim(Transform(SFT->FT_VFECPST,"@E 999,999,999.99")))
                                        Endif
                                    Endif
                                Endif
                            Endif
                          ...
                          IF (ExistTemplate("PROCMSG"))
                              aMens:=ExecTemplate("PROCMSG",.f.,.f.,{cAliasSD2})
                                ...
                                If len(aMens) > 0
                                    ...
                                    If MsSeek(cSD2Filial+(cAliasSD2)->D2_DOC+(cAliasSD2)->D2_SERIE+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD )
                        ...
                        MsSeek(cSFTFilial + cChaveD2 + "  " + (cAliasSD2)->D2_COD)
                        ...
                        If lNfCup .Or. (cAliasSD2)->D2_ORIGLAN $ "VD|LO"
                            ...
                            If SF2->F2_DESCONT > 0
                                If lFirstItem    // Somente faz o looping nos itens na primeira vez
                                    ...
                                    While !SD2->(Eof()) .And. cSD2Filial == SD2->D2_FILIAL .And.;
                        ...
                        If (.T. .or. !Empty((cAliasSD2)->D2_IDENTB6) .And. lNFPTER)
                             If SC5->C5_TIPO == "N"
                                 //--A7_FILIAL + A7_CLIENTE + A7_LOJA + A7_PRODUTO
                                 SA7->(dbSetOrder(1))
                                 If SA7->(MsSeek( cSA7Filial + (cAliasSD2)->(D2_CLIENTE+D2_LOJA+D2_COD) )) .and. !empty(SA7->A7_CODCLI) .and. !empty(SA7->A7_DESCCLI)
                            ...
                            ElseIf SC5->C5_TIPO == "B"
                                 ...
                                 SA5->(dbSetOrder(1))
                                 If SA5->(MsSeek( cSA5Filial + (cAliasSD2)->(D2_CLIENTE+D2_LOJA+D2_COD) )) .and. !empty(SA5->A5_CODPRF) .and. !empty(SA5->A5_DESREF)
                        ...
                        If lCpoMsgLT .And. lCpoLoteFor .And. SF4->F4_MSGLT $ "1"
                            cNumLotForn:=Alltrim(Posicione("SB8",2,cSB8Filial+(cAliasSD2)->D2_NUMLOTE+(cAliasSD2)->D2_LOTECTL+cCodProd,"B8_LOTEFOR"))
                        ...
                        If SM0->M0_ESTENT $ "PR/RJ/RS/" //TAG cBenef buscar o conteúdo da tabela 5.2 no sistema quando for do PR.
                            ...
                            dbSetOrder(4)
                            cCodlan:=""
                            If MsSeek(cCDVFilial +'S'+PadR('SPED',TamSX3("CDV_ESPECI")[1])+'S'+(cAliasSD2)->(D2_DOC+D2_SERIE+D2_CLIENTE+D2_LOJA+D2_ITEM))
                                ...
                                if CDV->(ColumnPos("CDV_NFE")) > 0
                                    ...
                                    dbSelectArea("CDY")
                                    dbSetOrder(1)
                                    If CDV->(ColumnPos("CDV_CODAJU")) > 0 .and. !Empty(CDV->CDV_CODAJU) .and. MsSeek( cCDYFilial + CDV->CDV_CODAJU)
                                        If CDY->CDY_NFE <> "2"
                                            lCodLan:=.T.
                                        EndIF
                                    EndIF
                        ...
                        Else
                            ...
                            If CDA->(ColumnPos("CDA_CODLAN")) > 0
                                dbSelectArea("CDA")
                                dbSetOrder(1)
                                cSeekCDA:=cCDAFilial + 'S' + PadR('SPED',TamSX3("CDA_ESPECI")[1]) + 'S' + (cAliasSD2)->(D2_DOC+D2_SERIE+D2_CLIENTE+D2_LOJA+D2_ITEM)
                        ...
                        If AliasIndic("D3E")
                            ...
                            dbSetOrder(1)
                            cIndEscala:=""
                            If MsSeek(PADR(cD3EFilial,TAMSX3("D3E_FILIAL")[1]) +(cAliasSD2)->D2_COD)
                        ...
                        ElseiF AliasIndic("CDL")
                            ...
                            DbSelectArea("CDL")
                            DbSetOrder(1)
                            DbSeek(cCDLFilial+(cAliasSD2)->D2_DOC+(cAliasSD2)->D2_SERIE+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA)
                            While !CDL->(Eof()) .And. CDL->CDL_FILIAL+CDL->CDL_DOC+CDL->CDL_SERIE+CDL->CDL_CLIENT+CDL->CDL_LOJA == cCDLFilial+(cAliasSD2)->D2_DOC+(cAliasSD2)->D2_SERIE+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA
                        ...
                        DbSelectArea("SF2")
                        DbSetOrder(1)
                        MsSeek(cSF2Filial+(cAliasSD2)->D2_DOC+(cAliasSD2)->D2_SERIE+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA)
                        dbSelectArea("CD2")
                        ...
                        DbSelectArea("SFT")
                        DbSetOrder(1)
                        If SFT->(DbSeek(cSFTFilial+"S"+(cAliasSD2)->(D2_SERIE+D2_DOC+D2_CLIENTE+D2_LOJA+PadR(D2_ITEM,TamSx3("FT_ITEM")[1])+D2_COD)))
                        ...
                        CD2->(dbSeek(cCD2Filial+"S"+SF2->F2_SERIE+SF2->F2_DOC+SF2->F2_CLIENTE+SF2->F2_LOJA+PadR((cAliasSD2)->D2_ITEM,4)+(cAliasSD2)->D2_COD))

                        While CD2->(!Eof()) .And. cCD2Filial == CD2->CD2_FILIAL .And.;
                            "S" == CD2->CD2_TPMOV .And.;
                            ...
                            /*DbSelectArea("SF7")
                            DbSetOrder(1)
                                If DbSeek(cSF7Filial+SB1->B1_GRTRIB+SA1->A1_GRPTRIB)
                                    If SF7->F7_BASEICM > 0
                            ...
                            Do Case
                                ...
                                Case AllTrim(CD2->CD2_IMP) == "ISS"
                                    ...
                                    If AliasIndic("CDN") .And. CDN->(dbSeek(cCDNFilial+cCodIss))
                                    ...
                                    /*If SF3->(FieldPos("F3_RECISS"))>0
                                        If SF3->F3_RECISS $"1S"
                                            If SF3->(dbSeek(cSF3Filial+SF2->F2_CLIENTE+SF2->F2_LOJA+SF2->F2_DOC+SF2->F2_SERIE))
                                                While !SF3->(EOF()) .And. cSF3Filial+SF3->F3_CLIEFOR+SF3->F3_LOJA+SF3->F3_NFISCAL+SF3->F3_SERIE==SF2->F2_FILIAL+SF2->F2_CLIENTE+SF2->F2_LOJA+SF2->F2_DOC+SF2->F2_SERIE
                      ...
                      //Tratamento para que o valor de PIS ST,COFINS ST venha a compor o valor total da nota.
                        ...
                        If (lCalSol .OR.  lMVCOMPET .OR. lDifal )
                            dbSelectArea("SF3")
                            dbSetOrder(4)
                            If MsSeek(cSF3Filial+SF2->F2_CLIENTE+SF2->F2_LOJA+SF2->F2_DOC+SF2->F2_SERIE)
                                ...
                                Elseif  lDifal
                                    If AliasInDic("F0L")
                                        dbSelectArea("F0L")
                                        dbSetOrder(1)
                                        If MsSeek(cF0LFilial+SF3->F3_ESTADO)    //F0L_FILIAL, F0L_UF, F0L_INSCR, R_E_C_N_O_, D_E_L_E_T_
                        ...
                        If SFT->(FieldPos("FT_CSTPIS")) > 0 .And. SFT->(FieldPos("FT_CSTCOF")) > 0
                            ...
                            If MsSeek(cSFTFilial+"S"+SF2->F2_SERIE+SF2->F2_DOC+SF2->F2_CLIENTE+SF2->F2_LOJA+PadR((cAliasSD2)->D2_ITEM,4)+(cAliasSD2)->D2_COD)
                ...
                // Msg Zona Franca de Manaus / ALC
                    dbSelectArea("SF3")
                    dbSetOrder(4)
                    dbSeek (cSF3Filial+SF2->F2_CLIENTE+SF2->F2_LOJA+SF2->F2_DOC+SF2->F2_SERIE)
                    Do While !SF3->(Eof()) .AND. cSF3Filial == SF3->F3_FILIAL .And.;
                    ...
                    If !SF2->F2_DESCZFR == 0 .or. ( lInfAdZF .and. nValBse > 0 )//Desnecessario seek redundante na SF3 pois o campo F2_DESCZFR ja possui os valores de ZFR de toda a venda
                        If lInfAdZF .And. (nValPisZF > 0 .Or. nValCofZF > 0)
                            cMensFis += MsgMaxLin("Descontos Ref. a Zona Franca de Manaus / ALC. ICMS - R$ "+str(nValBse-SF2->F2_DESCONT-nValPisZF-nValCofZF,13,2)+", PIS - R$ "+ str(nValPisZF,13,2) +"e COFINS - R$ " +str(nValCofZF,13,2))
                        ElseIF !lInfAdZF .And. (nValPisZF > 0 .Or. nValCofZF > 0)
                            cMensFis += MsgMaxLin("Desconto Ref. ao ICMS - Zona Franca de Manaus / ALC. R$ "+str(nValBse-SF2->F2_DESCONT-nValPisZF-nValCofZF,13,2))
                        Else
                            cMensFis += MsgMaxLin("Total do desconto Ref. a Zona Franca de Manaus / ALC. R$ "+str(nValBse-SF2->F2_DESCONT,13,2))
                        EndIF
                    EndIf
                EndIF
                ...
                If nValLeite > 0 .And. nPercLeite > 0
                    cMensFis += MsgMaxLin(Alltrim(Str(nPercLeite,10,2))+'% Incentivo à produção e à industrialização do leite = R$ '+ Alltrim(Str(nValLeite,10,2)))
                    aTotal[02] += nValLeite
                EndIf
                ...
                If Len(aIPIDev)>0
                    ...
                    EndDo
                        ...
                        For nX:=1 To Len(aIPIAux)
                            ...
                            cMensCli += MsgMaxLin("(Valor do IPI: R$ "+cValIPI+" - "+"Classificação fiscal: "+aIPIAux[nX][2]+") ")
                            cValIPI:=""
                            cNCMAux:=""
                        Next nX
                ...
                If nValSTAux > 0
                    ...
                    If lComplDev .And.  nBsCalcST == 0
                        cMensCli += MsgMaxLin("(Valor do ICMS ST: R$ "+cValST+") ")
                    Else
                        cMensCli += MsgMaxLin("(Base de Calculo do ICMS ST: R$ "+cBsST+ " - "+"Valor do ICMS ST: R$ "+cValST+") ")
                    EndIF
                    cValST:=""
                    cBsST:=""
                    nBsCalcST:=0
                ...
                If     nToTvBC > 0 .And. nToTvICMS > 0
                    cMensCli += MsgMaxLin("(Base de Calculo do ICMS : R$ "+AllTrim(Str(nToTvBC,15,2))+" - "+"Valor do ICMS : R$ "+AllTrim(Str(nToTvICMS,15,2))+") ")
                Endif
                ...
                If SM0->M0_ESTCOB $ "RS" .And. Len(aICMS) > 0 .And. Len(aICMSSt) > 0
                    cMensCli += MsgMaxLin(MsgCliRsIcm(aICMS,aICMSSt))
                Endif
                ...
                If aDest[9] $ "DF" .And. Len(aICMS) > 0 .And. Len(aICMSSt) > 0
                    cMensCli += MsgMaxLin(MsgCliDFIcm(aICMS,aICMSSt))
                Endif
                ...
                if nValICMParc > 0 .And. nBasICMParc > 0 .And. nValSTParc > 0 .And. nBasSTParc > 0
                    ...
                    cMensTmp:="Faturamento Direto ao Consumidor - Convenio ICMS Nº 51/00, de 15 de setembro de 2000. "
                    cMensTmp+="Base de calculo ICMS R$"+ AllTrim(Str(nBasICMParc,15,2))+" e "
                    cMensTmp+="Valor do ICMS R$"+ AllTrim(Str(nValICMParc,15,2))+". "
                    cMensTmp+="Base do ICMS-ST R$"+ AllTrim(Str(nBasSTParc,15,2))+" e "
                    cMensTmp+="Valor do ICMS-ST R$"+ AllTrim(Str(nValSTParc,15,2))+". "

                    If !Empty(aEntrega)
                        cMensTmp+="Concessionaria que ira entregar o veiculo ao adquirente "+ConvType(aEntrega[09],115)+". "
                        cMensTmp+="CNPJ: "+AllTrim(aEntrega[01])+" e IE: "+AllTrim(aEntrega[10])+". "
                        cMensTmp+="Endereço: "+ConvType(aEntrega[02],125)+", "+ConvType(aEntrega[03],10)+" "+ConvType(aEntrega[04],60)+". " //Rua,Num,Complemento
                        cMensTmp+=ConvType(aEntrega[05],60)+" - "+ ConvType(aEntrega[07],50) +"-"+ConvType(aEntrega[08],2)+". "//Bairro, Cidade, UF
                    Else
                        cMensTmp+="Concessionaria que ira entregar o veiculo ao adquirente "+ConvType(aDest[02],115)+". "
                        cMensTmp+="CNPJ "+AllTrim(aDest[01])+" e IE: "+AllTrim(aDest[14])+". "
                        cMensTmp+="Endereço: "+ConvType(aDest[03],125)+" "+ConvType(aDest[04],10)+" "+ConvType(aDest[05],60)+", " //Rua,Num,Complemento
                        cMensTmp+=ConvType(aDest[06],60)+ ", "+ ConvType(aDest[08],50) +" - "+ConvType(aDest[09],2)+". "//Bairro, Cidade, UF
                    EndIF
                ...
                If ((SubStr(SM0->M0_CODMUN,1,2) == "35" ) .and. "REMESSA POR CONTA E ORDEM DE TERCEIROS" $ Upper(cNatOper) .and. lOrgaoPub )
                    cMensTmp:="NF-e emitida nos termos do artigo 129-A do RICMS."
                    cMensTmp+="(Redacao dada ao artigo pelo Decreto n60.060 , de 14.01.2014, DOE SP de 15.01.2014)"
                    cMensFis+=MsgMaxLin(cMensTmp)
                EndIf
                ...
                if lBrinde
                    ...
                    cMensCli += MsgMaxLin("Nota Fiscal emitida nos termos do inciso II do art. 456 do RICMS - Nota Fiscal de Aquisição nº "+aDocDat[2]+", de "+aDocDat[1]+".")
                endif
                //Tratamento para incluir a mensagem em informacoes adicionais do FECP -DF - MG - PR - RJ - RS.
                If nValTFecp > 0
                    If cVerAmb >= "4.00"
                    cMensFis += MsgMaxLin(NfeMFECOP(nValTFecp,aDest[9],"1",aICMS,aICMSST,cVerAmb))
                    Else
                        cMensCli += MsgMaxLin(NfeMFECOP(nValTFecp,aDest[9],"1",aICMS,aICMSST,cVerAmb))
                    EndIf
                EndIf
            EndIf
        EndIf
...
Else
    dbSelectArea("SF1")
    dbSetOrder(1)
    If MsSeek(cSF1Filial+cNota+cSerie+cClieFor+cLoja)
        //ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
        //³Tratamento temporario do CTe                                            ³
        //ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
        ...
        Else
            ...
            If SF1->F1_TIPO $ "DB"
                dbSelectArea("SA1")
                dbSetOrder(1)
                MsSeek(cSA1Filial+cClieFor+cLoja)
                ...
                If lF1Motivo .AND. AllTrim(SF1->F1_ORIGLAN) == "LO" .AND. LjAnalisaLeg(73)[1] .AND. !Empty(SF1->F1_MOTIVO)
                    cMensFis += MsgMaxLin(SF1->F1_MOTIVO)
                EndIf
                ...
                If SF1->(FieldPos("F1_FORRET"))<>0 .And. !Empty(SF1->F1_FORRET+SF1->F1_LOJARET) .And. SF1->F1_FORRET+SF1->F1_LOJARET <> SF1->F1_FORNECE+SF1->F1_LOJA
                    dbSelectArea("SA1")
                    dbSetOrder(1)
                    IF MsSeek(cSA1Filial+SF1->F1_FORRET+SF1->F1_LOJARET)
                ...
                If SF1->(FieldPos("F1_FORENT")) <> 0 .And. !Empty(SF1->F1_FORENT+SF1->F1_LOJAENT) .And. SF1->F1_FORENT+SF1->F1_LOJAENT <> SF1->F1_FORNECE+SF1->F1_LOJA
                    dbSelectArea("SA1")
                    dbSetOrder(1)
                    If MsSeek(cSA1Filial+SF1->F1_FORENT+SF1->F1_LOJAENT)
                ...
                */
                ...
                Else
                    MsSeek(cSA1Filial+SF1->F1_FORNECE+SF1->F1_LOJA)
                    ...
                    If !lUsaCliEnt
                        ...
                        If !Empty(AllTrim(SF1->F1_FORENT)) .And. !Empty(AllTrim(SF1->F1_LOJAENT))
                            If Len(aEntrega) > 0
                                ...
                                //rejeicao 523 - CFOP não é de Operação Estadual e UF emitente igual à UF destinatário
                                If aEntrega[08] <> IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) .And. !lCNPJIgual //aEntrega[08] <> Upper(SA1->A1_EST)
                                    MsSeek(cSA1Filial+SF1->F1_FORENT+SF1->F1_LOJAENT)
                                EndIf
                                //Se a UF de entrega for igual a UF do emitente (Operação interna) - busco os dados do cliente para montar como destinatario.
                                //Se o CNPJ do emitente for igual ao do destinatário também levo os dados do cliente, mesmo que UFs forem diferente.
                                //Se o cliente não for consumidor final e possuir IE, pode ocorrer a rejeição 773 - Operação Interna e UF de destino difere da UF do emitente
            ...
            Else
                   dbSelectArea("SA2")
                dbSetOrder(1)
                MsSeek(cSA2Filial+cClieFor+cLoja)
                If SF1->( ColumnPos( "F1_DEVMERC" ) ) > 0
                    cDevMerc:=Alltrim(SF1->F1_DEVMERC)
                EndIf
               ...
               If SF1->(FieldPos("F1_FORRET"))<>0 .And. !Empty(SF1->F1_FORRET+SF1->F1_LOJARET) .And. SF1->F1_FORRET+SF1->F1_LOJARET<>SF1->F1_FORNECE+SF1->F1_LOJA
                    dbSelectArea("SA2")
                    dbSetOrder(1)
                    If MsSeek(cSA2Filial+SF1->F1_FORRET+SF1->F1_LOJARET)
                        ...
                        aadd(aRetirada,IIF(Empty(SA2->A2_PAIS),"1058"  ,Posicione("SYA",1,cSYAFilial+SA2->A2_PAIS,"YA_SISEXP")))
                        aadd(aRetirada,IIF(Empty(SA2->A2_PAIS),"BRASIL",Posicione("SYA",1,cSYAFilial+SA2->A2_PAIS,"YA_DESCR" )))
                ...
                If SF1->(FieldPos("F1_FORENT")) <> 0 .And. !Empty(SF1->F1_FORENT+SF1->F1_LOJAENT) .And. SF1->F1_FORENT+SF1->F1_LOJAENT <> SF1->F1_FORNECE+SF1->F1_LOJA
                    dbSelectArea("SA2")
                    dbSetOrder(1)
                    If MsSeek(cSA2Filial+SF1->F1_FORENT+SF1->F1_LOJAENT)
                        ...
                        aadd(aEntrega,IIF(Empty(SA2->A2_PAIS),"1058"  ,Posicione("SYA",1,cSYAFilial+SA2->A2_PAIS,"YA_SISEXP")))
                        aadd(aEntrega,IIF(Empty(SA2->A2_PAIS),"BRASIL",Posicione("SYA",1,cSYAFilial+SA2->A2_PAIS,"YA_DESCR" )))
            ...
            Else
                dbSelectArea("SA2")
                dbSetOrder(1)
                MsSeek(cSA2Filial+SF1->F1_FORNECE+SF1->F1_LOJA)
            EndIf
            ...
            SF3->(dbSetOrder(5))
            if ( SF3->(dbSeek(cSF3Filial+cSerie+cNota)) )
                ...
                if nValIPIDestac > 0
                    cMensFis += MsgMaxLin("Valor do IPI: R$ " + AllTrim(Transform(nValIPIDestac, "@ze 9,999,999,999,999.99")) + " ")
                endif
            ...
            If !Empty(SF1->F1_DUPL)
                ...
                #ELSE
                    MsSeek(cSE2Filial+SF1->F1_PREFIXO+SF1->F1_DOC)
                #ENDIF
                While !Eof() .And. cSE2Filial == (cAliasSE2)->E2_FILIAL .And.;
            ...
            If FieldPos("F1_TRANSP") > 0 .And. !Empty(SF1->F1_TRANSP)
                dbSelectArea("SA4")
                dbSetOrder(1)
                MsSeek(cSA4Filial+SF1->F1_TRANSP)
            ...
            #ELSE
                MsSeek(cSD1Filial+SF1->F1_DOC+SF1->F1_SERIE+SF1->F1_FORNECE+SF1->F1_LOJA)
            #ENDIF

            nCountIT:=0
            While !Eof() .And. cSD1Filial == (cAliasSD1)->D1_FILIAL .And.;
                ...
                dbSelectArea("SF4")
                dbSetOrder(1)
                MsSeek(cSF4Filial+(cAliasSD1)->D1_TES)
                ...
                If SF1->(FieldPos("F1_MENNOTA"))>0
                    If !AllTrim(SF1->F1_MENNOTA) $ cMensCli
                        ...
                        If !Empty(cFieldMsg) .and. SF1->(FieldPos(cFieldMsg)) > 0 .and. !Empty(&("SF1->"+cFieldMsg))
                            cMensCli:=MsgMaxLin(alltrim(&("SF1->"+cFieldMsg)))
                        Else
                            cMensCli += MsgMaxLin(AllTrim(SF1->F1_MENNOTA))
                        EndIf
                    EndIf
                EndIf
                ...
                If SF1->(FieldPos("F1_MENPAD"))>0
                    If !Empty(SF1->F1_MENPAD) .And. !AllTrim(FORMULA(SF1->F1_MENPAD)) $ cMensFis
                        cMensFis += MsgMaxLin(AllTrim(FORMULA(SF1->F1_MENPAD)))
                    EndIf
                EndIf
                ...
                DbSelectArea("SFT")
                DbSetOrder(1)
                IF SFT->(DbSeek(cSFTFilial+"E"+(cAliasSD1)->(D1_SERIE+D1_DOC+D1_FORNECE+D1_LOJA)))
                ...
                If SF1->(FieldPos("F1_STATUS"))>0 .And.SD1->(FieldPos("D1_TESACLA"))>0 .And. SF1->F1_STATUS='C'
                    MsSeek(cSF4Filial+(cAliasSD1)->D1_TESACLA)
                Else
                    MsSeek(cSF4Filial+(cAliasSD1)->D1_TES)
                EndIf
                ...
                SFT->( dbSetOrder( 1 ) )
                //utiliza a funcao SpedNatOper ( SPEDXFUN ) que possui o tratamento para a natureza da operacao/prestacao
                if FindFunction( "SpedNatOper" ) .And. SFT->( MsSeek( cSFTFilial + cChaveD1 ) )
                    If !Alltrim(SpedNatOper( nil , lNatOper , "SFT" , "SF4" , .T. )[ 2 ])$cNatOper
                        If    Empty(cNatOper)
                            cNatOper:=SpedNatOper( nil , lNatOper , "SFT" , "SF4" , .T. )[ 2 ]
                        Else
                ...
                If lMvImpFecp
                    If (lValFecp .Or. lVfecpst)
                        DbSelectArea("SFT")
                        DbSetOrder(1)
                        If SFT->(DbSeek((cSFTFilial + cChaveD1 )))
                            nValTFecp += SFT->FT_VFECPST + SFT->FT_VALFECP  + SFT->FT_VFECPMG + SFT->FT_VFESTMG
                            nValIFecp:=SFT->FT_VFECPST + SFT->FT_VALFECP  + SFT->FT_VFECPMG + SFT->FT_VFESTMG
                        Endif
                ...
                if SF4->F4_AJUSTE =="S" .and. aDest[1] == SM0->M0_CGC .and. (cAliasSD1)->(D1_TIPO) == "D" .and. (cAliasSD1)->D1_FORMUL == "S"
                    ...
                    dbSelectArea("SF2")
                    dbSetOrder(1)
                    if SF2->(DbSeek(cSF2Filial+(cAliasSD1)->(D1_NFORI)+(cAliasSD1)->(D1_SERIORI)))
                        dbSelectArea("SA1")
                        dbSetOrder(1)
                        if SA1->(DbSeek(cSA1Filial+(SF2->F2_CLIENTE)+(SF2->F2_LOJA)))
                            cMensTmp:=iIf(!Empty(SA1->A1_CGC),'CNPJ: '+Rtrim(SA1->A1_CGC) ,'')
                            cMensTmp+=iIf (!Empty(SA1->A1_NOME),' NOME: '+Rtrim(SA1->A1_NOME) ,'')
                            cMensTmp+=iIf (!Empty(SA1->A1_END),' ENDEREÇO: '+Rtrim(SA1->A1_END) ,'')
                            cMensTmp+=iIf (!Empty(SA1->A1_BAIRRO),' BAIRRO: '+Rtrim(SA1->A1_BAIRRO) ,'')
                            cMensTmp+=iIf (!Empty(SA1->A1_EST),' UF: '+Rtrim(SA1->A1_EST) ,'')
                            if !Empty(SA1->A1_PAIS)
                                dbSelectArea("SYA")
                                dbSetOrder(1)
                                if SYA->(DbSeek(cSYAFilial+(SA1->A1_PAIS)))
                                    cMensTmp += iIf (!Empty(SYA->YA_DESCR),' PAIS: '+Rtrim(SYA->YA_DESCR),'')
                                endif
                            endif
                            cMensCli += MsgMaxLin(cMensTmp)
                        endif
                ...
                If !Empty((cAliasSD1)->D1_NFORI)
                    ...
                    If !MsSeek(cSD1Filial+cSeekD1) .and. ((cAliasSD1)->D1_TIPO) $ "C|I"
                        cSeekD1:=cSeekAux
                    EndIf
                    ...
                    If MsSeek(cSD1Filial+cSeekD1)
                        ...
                        SF1->( dbSetOrder( 1 ) )
                        SF1->( msSeek( cSF1Filial + SD1->D1_DOC + SD1->D1_SERIE + SD1->D1_FORNECE + SD1->D1_LOJA + SD1->D1_TIPO ) )
                        lSeekOk:=.T.
                        If SD1->D1_TIPO $ "DB"
                            dbSelectArea("SA1")
                            dbSetOrder(1)
                            MsSeek(cSA1Filial+SD1->D1_FORNECE+SD1->D1_LOJA)
                        Else
                            dbSelectArea("SA2")
                            dbSetOrder(1)
                            MsSeek(cSA2Filial+SD1->D1_FORNECE+SD1->D1_LOJA)
                    ...
                    Else
                        ...
                        IF (cAliasSD1)->D1_ORIGLAN =="LO"
                            If (cAliasSD1)->(FieldPos("D1_FILORI")) > 0
                                cFilDev:=Iif(Empty((cAliasSD1)->D1_FILORI),cSD2Filial,(cAliasSD1)->D1_FILORI)
                            Else
                                cFilDev:=cSD2Filial
                            EndIf
                        ...
                        ElseIf (cAliasSD1)->D1_TIPO == "N" .And. (cAliasSD1)->D1_FORMUL = "S"
                            dbSelectArea("SFT")
                               dbSetOrder(4)
                               If MsSeek(cSFTFilial+"E"+(cAliasSD1)->D1_FORNECE+(cAliasSD1)->D1_LOJA+(cAliasSD1)->D1_SERIORI+(cAliasSD1)->D1_NFORI)
                                   dbSelectArea("SF3")
                                   dbSetOrder(4)
                                   If MsSeek(cSF3Filial+SFT->FT_CLIEFOR+SFT->FT_LOJA+SFT->FT_NFISCAL+SFT->FT_SERIE)
                ...
                Else
                    ...
                    If Alltrim( (cAliasSD1)->D1_ORIGLAN ) $ "D-DP-FR" .And. Alltrim( (cAliasSD1)->D1_TIPO ) == "C"
                        dbSelectArea("SF8")
                        dbSetOrder(3)
                        If dbSeek(cChavesf8:=cSF8Filial+(cAliasSD1)->D1_DOC+(cAliasSD1)->D1_SERIE+(cAliasSD1)->D1_FORNECE+(cAliasSD1)->D1_LOJA)
                            aAreaSD1:=SD1->(GetArea())
                            aAreaSF1:=SF1->(GetArea())
                            Do While cChavesf8 == SF8->(F8_FILIAL+F8_NFDIFRE+F8_SEDIFRE+F8_TRANSP+F8_LOJTRAN)
                                dbSelectArea("SD1")
                                dbSetOrder(1)
                                If dbSeek(cSD1Filial+SF8->F8_NFORIG+SF8->F8_SERORIG+SF8->F8_FORNECE+SF8->F8_LOJA)
                                    dbSelectArea("SF1")
                                    dbSetOrder(1)
                                    If dbSeek(cSF1Filial+SD1->D1_DOC+SD1->D1_SERIE+SD1->D1_FORNECE+SD1->D1_LOJA+SD1->D1_TIPO)
                ...
                dbSelectArea("SB1")
                dbSetOrder(1)
                MsSeek(cSB1Filial+(cAliasSD1)->D1_COD)
                //Veiculos Novos
                If AliasIndic("CD9")
                    dbSelectArea("CD9")
                    dbSetOrder(1)
                    MsSeek(cCD9Filial+cChaveD1)
                EndIf
                //Combustivel
                If AliasIndic("CD6")
                    dbSelectArea("CD6")
                    dbSetOrder(1)
                    MsSeek(cCD6Filial+cChaveD1)
                EndIf
                //Medicamentos
                If AliasIndic("CD7")
                    dbSelectArea("CD7")
                    dbSetOrder(1)
                    MsSeek(cCD7Filial+cChaveD1)
                EndIf
                // Armas de Fogo
                If AliasIndic("CD8")
                    dbSelectArea("CD8")
                    dbSetOrder(1)
                    MsSeek(cCD8Filial+cChaveD1)
                EndIf
                //Anfavea
                If lAnfavea
                    dbSelectArea("CDR")
                    dbSetOrder(1)
                    MsSeek(cCDRFilial+"S"+(cAliasSD1)->D1_DOC+(cAliasSD1)->D1_SERIE+(cAliasSD1)->D1_FORNECE+(cAliasSD1)->D1_LOJA)

                    dbSelectArea("CDS")
                    dbSetOrder(1)
                    MsSeek(cCDSFilial+"S"+(cAliasSD1)->D1_SERIE+(cAliasSD1)->D1_DOC+(cAliasSD1)->D1_FORNECE+(cAliasSD1)->D1_LOJA+(cAliasSD1)->D1_ITEM+(cAliasSD1)->D1_COD)
                EndIf
                //RASTREABILIDADDE
                 If AliasIndic("F0A")
                    dbSelectArea("F0A")
                    dbSetOrder(1)
                    MsSeek(cF0AFilial+cChaveD1)
                EndIf

                dbSelectArea("SB5")
                dbSetOrder(1)
                If MsSeek(cSB5Filial+(cAliasSD1)->D1_COD)
                    If SB5->(FieldPos("B5_DESCNFE")) > 0 .And. !Empty(SB5->B5_DESCNFE)
                        cInfAdic:=Alltrim(SB5->B5_DESCNFE)
                    Else
                        cInfAdic:=""
                ...
                MsSeek(cSFTFilial+"E"+(cAliasSD1)->D1_SERIE+(cAliasSD1)->D1_DOC+(cAliasSD1)->D1_FORNECE+(cAliasSD1)->D1_LOJA+(cAliasSD1)->D1_ITEM+(cAliasSD1)->D1_COD)
                If SFT->(FieldPos("FT_DS43080")) <> 0 .And. SFT->FT_DS43080 > 0 .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "MG"
                    nDescRed:=SFT->FT_DS43080
                    nDesTotal+= nDescRed
                EndIF
               ...
               EndIf
                ...
                If (.T. .or. !Empty((cAliasSD1)->D1_IDENTB6) .And. lNFPTER)
                    If (cAliasSD1)->D1_TIPO == "N"
                        //--A5_FILIAL + A5_FORNECE + A5_LOJA + A5_PRODUTO
                        SA5->(dbSetOrder(1))
                        If SA5->(MsSeek( cSA5Filial + (cAliasSD1)->(D1_FORNECE+D1_LOJA+D1_COD) )) .and. !empty(SA5->A5_CODPRF) .and. !empty(SA5->A5_DESREF)
                            cCodProd:=SA5->A5_CODPRF
                            cDescProd:=SA5->A5_DESREF
                        EndIf
                    ElseIf (cAliasSD1)->D1_TIPO == "B"
                     //--A7_FILIAL + A7_CLIENTE + A7_LOJA + A7_PRODUTO
                        SA7->(dbSetOrder(1))
                        If SA7->(MsSeek( cSA7Filial + (cAliasSD1)->(D1_FORNECE+D1_LOJA+D1_COD) )) .and. !empty(SA7->A7_CODCLI) .and. !empty(SA7->A7_DESCCLI)
                            cCodProd:=SA7->A7_CODCLI
                            cDescProd:=SA7->A7_DESCCLI
                        EndIf
                    EndIf
                ...
                If SM0->M0_ESTENT $ "PR/RJ/RS/" //TAG cBenef buscar o conteúdo da tabela 5.2 no sistema quando for do PR.
                    ...
                    dbSetOrder(4)
                    cCodlan:=""
                    If MsSeek(cCDVFilial +'E'+PadR('SPED',TamSX3("CDV_ESPECI")[1])+'S'+(cAliasSD1)->(D1_DOC+D1_SERIE+D1_FORNECE+D1_LOJA+D1_ITEM))
                ...
                Else
                    ...
                    If CDA->(ColumnPos("CDA_CODLAN")) > 0
                        dbSelectArea("CDA")
                        dbSetOrder(1)
                        cSeekCDA:=cCDAFilial +'E'+PadR('SPED',TamSX3("CDA_ESPECI")[1])+'S'+(cAliasSD1)->(D1_DOC+D1_SERIE+D1_FORNECE+D1_LOJA+D1_ITEM)
                        If MsSeek(cSeekCDA) //CDA_FILIAL, CDA_TPMOVI, CDA_ESPECI, CDA_FORMUL, CDA_NUMERO, CDA_SERIE, CDA_CLIFOR, CDA_LOJA, CDA_NUMITE, CDA_SEQ, CDA_CODLAN, CDA_CALPRO
                            While cSeekCDA == CDA->(CDA_FILIAL + CDA_TPMOVI + CDA_ESPECI + CDA_FORMUL + CDA_NUMERO + CDA_SERIE + CDA_CLIFOR + CDA_LOJA + CDA_NUMITE)
                ...
                If AliasIndic("D3E")
                    ...
                    dbSetOrder(1)
                    cIndEscala:=""
                    If MsSeek(PADR(cD3EFilial,TAMSX3("D3E_FILIAL")[1]) +(cAliasSD1)->D1_COD)
                        If D3E->(ColumnPos("D3E_INDESC")) > 0
                            If    !Empty(D3E->D3E_INDESC)  .AND.  D3E->D3E_INDESC == "1"
                                cIndEscala:="S"
                            EndIF
                ...
                DbSelectArea("SC7")
                DbSetOrder(1)
                If MsSeek(cSC7Filial+(cAliasSD1)->D1_PEDIDO+(cAliasSD1)->D1_ITEM)
                ...
                DbSelectArea("SF2")
                DbSetOrder(1)
                If MsSeek(cSF2Filial+(cAliasSD1)->D1_NFORI+(cAliasSD1)->D1_SERIORI)
                ...
                ElseIf !lEasy .Or. (lEasy .AND. (cAliasSD1)->D1_TIPO $ "DCPI")
                    ...
                    If MsSeek(cCD5Filial+(cAliasSD1)->D1_DOC+(cAliasSD1)->D1_SERIE+(cAliasSD1)->D1_FORNECE+(cAliasSD1)->D1_LOJA+(cAliasSD1)->D1_ITEM)
                            aAdd(aDI,{;
                ...
                If SFT->(DbSeek(cSFTFilial+"E"+(cAliasSD1)->(D1_SERIE+D1_DOC+D1_FORNECE+D1_LOJA+D1_ITEM+D1_COD)))
                   aadd(aCSTIPI,{SFT->FT_CTIPI})
                   //TRATAMENTO DA AQUISIÇÃO DE LEITE DO PRODUTOR RURAL CONFORME ARTIGO 207-B, INCISO II RICMS/MG
                   //PEGA OS VALORES E PERCENTUAL DO INNCENTIVO NOS ITENS NA SFT.
                   If SFT->(FieldPos("FT_PRINCMG")) > 0 .And. SFT->(FieldPos("FT_VLINCMG")) > 0
                ...
                SF1->(MsSeek(cSF1Filial+(cAliasSD1)->(D1_DOC+D1_SERIE+D1_FORNECE+D1_LOJA+D1_TIPO)))
                CD2->(MsSeek(cCD2Filial+"E"+SF1->F1_SERIE+SF1->F1_DOC+SF1->F1_FORNECE+SF1->F1_LOJA+PadR((cAliasSD1)->D1_ITEM,4)+(cAliasSD1)->D1_COD))
                While !CD2->(Eof()) .And. cCD2Filial == CD2->CD2_FILIAL .And.;
                    "E" == CD2->CD2_TPMOV .And.;
                    ...
                    IF !(cAliasSD1)->D1_TIPO $ "DB"
                        If SA2->(DbSeek(cSA2Filial+SF1->F1_FORNECE+SF1->F1_LOJA))
                            If SF7->(DbSeek(cSF7Filial+SB1->B1_GRTRIB+SA2->A2_GRPTRIB))
                        ...
                        EndIf
                    Else
                        If SA1->(DbSeek(cSA1Filial+SF1->F1_FORNECE+SF1->F1_LOJA))
                            If SF7->(DbSeek(cSF7Filial+SB1->B1_GRTRIB+SA1->A1_GRPTRIB))
                                If  SF7->F7_BASEICM > 0 .And. SF7->F7_BASEICM < 100
                                    nMargem:=100 - SF7->F7_BASEICM
                                EndIf
                            EndIf
                    ...
                    Do Case
                        ...
                        Case AllTrim(CD2->CD2_IMP) == "ISS"
                            ...
                            // If SF3->(FieldPos("F3_RECISS"))>0
                            //     If SF3->F3_RECISS $"1|S"
                            //         If SF3->(dbSeek(cSF3Filial+SF1->F1_FORNECE+SF1->F1_LOJA+SF1->F1_DOC+SF1->F1_SERIE))
                            //             While !SF3->(EOF()) .And. cSF3Filial+SF3->F3_CLIEFOR+SF3->F3_LOJA+SF3->F3_NFISCAL+SF3->F3_SERIE==SF1->F1_FILIAL+SF1->F1_FORNECE+SF1->F1_LOJA+SF1->F1_DOC+SF1->F1_SERIE
                            //                 If SF3->F3_TIPO=="S" //Serviço
                            //                     nValISSRet+= SF3->F3_VALICM
                            //                 EndIf
                            //                 SF3->(dbSkip())
                ...
                dbSelectArea("SFT") //Livro Fiscal Por Item da NF
                dbSetOrder(1) //FT_FILIAL+FT_TIPOMOV+FT_SERIE+FT_NFISCAL+FT_CLIEFOR+FT_LOJA+FT_ITEM+FT_PRODUTO
                If MsSeek(cSFTFilial+"E"+SF1->F1_SERIE+SF1->F1_DOC+SF1->F1_FORNECE+SF1->F1_LOJA+PadR((cAliasSD1)->D1_ITEM,4)+(cAliasSD1)->D1_COD) .And. ;
            ...
            If nValIcmDev > 0 .And. nValIcmDif > 0
                cMensTmp:="Operacao com diferimento parcial de 66,66% do imposto no valor de R$ " + Alltrim(Str(nValIcmDif,10,2)) + " - "
                cMensTmp+="ICMS devido de R$ " + Alltrim(Str(nValIcmDev,10,2)) + ", "
                cMensTmp+="nos termos do Art 459 do DECRETO N.º 7.871/2017 - RICMS/PR" //ISSUE DSERTSS1-6543 - Decreto 7.871 que revoga o regulamento do ICMS aprovado pelo decreto n 6080 de 28 de setembro de 2012.
                cMensFis+=MsgMaxLin(cMensTmp)
            Endif
            ...
            If     lIcmsPR .And. nToTvBC > 0 .And. nToTvICMS > 0
                cMensCli += MsgMaxLin("(Base de Calculo do ICMS : R$ "+nToTvBC+ " - "+"Valor do ICMS : R$ "+nToTvICMS+") ")
            Endif
...
If FunName() <> "SPEDNFSE"
    IF cVeramb >= "4.00"
       //Ajute para alimentar o aDetPag
        ...
        ElseIf (lVLojaDir .OR. !Empty(POSICIONE("SL1",2,cSL1Filial+SF2->F2_SERIE+SF2->F2_DOC,"L1_NUM"))) .And. cTipo == "1" .And. ( aRetPgLoj:=GetPagLoja(aDupl, cChvPag, aTotal[2], cIndPag,cVerAmb) )[1]
...
If !Empty(aNota)
    ...
    For nA:=1 to Len(aMensAux)
        cMensFis += MsgMaxLin(aMensAux[nA]+CRLF)
    Next

    If cMensONU <> ""
        cMensCli += MsgMaxLin(Alltrim(cMensONU))
    EndIf
...
If len(aEEC) > 0
    For nY:=1 To Len(aEEC)
           dbSelectArea("SF1")
        dbSetOrder(1)
        If DbSeek(cSF1Filial+aEEC[ny][2][1]+aEEC[ny][2][2]+aEEC[ny][2][3]+aEEC[ny][2][4])
            If cValidCh <> (cSF1Filial+aEEC[ny][2][1]+aEEC[ny][2][2]+aEEC[ny][2][3]+aEEC[ny][2][4])
                cCfor:=SF1->F1_FORNECE
                cLojaEn:=SF1->F1_LOJA
                dEmisEn:=SF1->F1_EMISSAO
                cDocEn:=SF1->F1_DOC
                cSerieEn:=SF1->F1_SERIE
                cValidCh:=(cSF1Filial+aEEC[ny][2][1]+aEEC[ny][2][2]+aEEC[ny][2][3]+aEEC[ny][2][4])

                dbSelectArea("SA2")
                dbSetOrder(1)
                If DbSeek(cSA2Filial+cCfor+cLojaEn)
                    cCnpjen:=SA2->A2_CGC
                EndIf

                dbSelectArea( "SD1" )
                dbSetOrder( 1 )
                cChave1:=cSD1Filial + cDocEn + cSerieEn + cCfor + cLojaEn

                if( dbSeek( cChave1 ) )
                    dbSelectArea( "SB1" )
                       dbSetOrder( 1 )

                    while !SD1->(eof()) .and. cChave1 == cSD1Filial + SD1->D1_DOC + SD1->D1_SERIE + SD1->D1_FORNECE + SD1->D1_LOJA
                        if( dbSeek( cSB1Filial + SD1->D1_COD ) )
...
If Len(cMsgCli)>0 .and. !Empty(cMsgCli)
    ...
    IF (SM0->M0_ESTENT) $ "SP" .AND. cTipo=='0' .And. !Empty(cSerRefcup + cNfRefcup)
        SFT->(dbSetOrder(6))
        If SFT->(dbSeek(cSFTFilial+"S"+cNfRefcup+cSerRefcup))
...
If SC6->( FieldPos("C6_PEDVINC") ) > 0 .And. !Empty(SC6->C6_PEDVINC)
   ...
   SC5->( dbSetOrder(1) )
    ...
    If SC5->( MsSeek( cChave:=cSC5Filial + cPedVinc ) )
    ...
    If lAtuSC5 .And. SC5->( MsSeek( cSC5Filial + cNumPed )   )
...
Local aNfViRuNFP:={}
    ...
    SD1->(dbSetOrder(1))
        If SD1->(dbSeek(cSD1Filial+SD1->D1_NFORI+SD1->D1_SERIORI))//D1_NFORI,D1_SERIORI
                   SF1->(dbSetOrder(1))
                   If SF1->(dbSeek(cSF1Filial+SD1->D1_DOC+SD1->D1_SERIE+SD1->D1_FORNECE+SD1->D1_LOJA) .And. AllTrim(SF1->F1_ESPECIE)=="NFP")
...
dbSelectArea("SB8")
dbSetOrder(2)
if MsSeek(cSB8Filial+cNumLote+cLoteCtl+cProduto)
...
If lProdItem
    ...
    If dbSeek( cSB1Filial + AllTrim( aProd[2] ) )
...
If len(aProd) > 36 .and. len(aNota) > 1 .and. MsSeek( cSD2Filial + PADR(aNota[2],TAMSX3("D2_DOC")[1]) + PADR(aNota[1],TAMSX3("D2_SERIE")[1]))
...
IF cTipo == "1" .And. cF2Tipo == "C" .And. len (aProd) > 0 .And. SF2->F2_TPCOMPL <> "2"
    ...
    If SD2->( DbSeek(cSD2Filial+ SD2->D2_DOC + SD2->D2_SERIE ))
        If !Empty(SD2->D2_NFORI).And. !Empty(SD2->D2_SERIORI) .And. !Empty(SD2->D2_ITEMORI)
            DbSelectArea("SF2")
            DbSetOrder(1)//F2_FILIAL, F2_DOC, F2_SERIE, F2_CLIENTE, F2_LOJA, F2_FORMUL, F2_TIPO,
            IF SF2->( DbSeek(cSF2Filial+ SD2->D2_NFORI + SD2->D2_SERIORI ))
...
Else
    dbselectarea("SL1")
    SL1->(dbsetorder(2))
    If SL1->(dbseek(cSL1Filial + SF2->F2_SERIE + SF2->F2_DOC))
        ...
        If lCondCN .And. SL4->(dbseek(SL1->L1_FILIAL + SL1->L1_NUM))
            isloja:=.T.
            While !SL4->(Eof()) .And. cSL4Filial == SL4->L4_FILIAL .And. SL1->L1_NUM == SL4->L4_NUM
                ...
                If cTpIntegra == "1"
                    ...
                    If !Empty(cAdmFin)
                        ...
                        aAreaSA1:=SA1->(GetArea())
                            ...
                            If SA1->(MsSeek(cSA1Filial + cAdmFin)) .And. !Empty(SA1->A1_CGC)
                                cA1Cgc:=SA1->A1_CGC
                            Else
                                cA1Cgc:=""
                            EndIf
...
If SC5->(MsSeek(cSC5Filial+aNfVinc[1][9]))
    cNEmp:=Iif(SC5->(FieldPos("C5_NTEMPEN")) > 0,Alltrim(SC5->C5_NTEMPEN),"")
EndIf

If SC6->(MsSeek(cSC6Filial+aNfVinc[1][9]))
...
Static Function RetInfoSBZ(cProduto, cUmDipi, nConvDip)
    ...
    If SBZ->( DbSeek(cSBZFilial + cProduto) )
...
static function MsgMaxLin(cMsg as character) as character
	local cMsgTmp as character
	local cMsgMax as character
	local cPadChr as character
	local nMsgTmp as numeric
	local nMsgMax as numeric
	begin sequence
		cMsgTmp:=allTrim(cMsg)
		if (empty(cMsgTmp))
			cMsgMax:=cMsgTmp
			break
		endif
		...
		nMsgTmp:=len(cMsgTmp)
		if (nMsgTmp<nMsgMax)
			cMsgMax:=PADR(cMsgTmp,nMsgMax,cPadChr)
			cMsgMax+=" "
			break
		endif
		cMsgMax:=""
		while (nMsgTmp>=nMsgMax)
			cMsgMax+=subStr(cMsgTmp,1,nMsgMax)
			cMsgMax+=" "
			cMsgTmp:=subStr(cMsgTmp,nMsgMax+1)
			nMsgTmp-=nMsgMax
		end while
		if (!empty(cMsgTmp))
			cMsgMax:=PADR(cMsgTmp,nMsgMax,cPadChr)
