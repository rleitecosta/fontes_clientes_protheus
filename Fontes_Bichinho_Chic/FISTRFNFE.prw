...
???????????????????????????????????????
???????????????????????????????????????
??rograma  ?FISTRFNFE?Autor ?        ?Data ? 30/07/12   ??
???????????????????????????????????????
??escri?o ?Ponto de Entrada para impressao da CC-e.                   ??
...
//////////////////////////////////////////////////////////////
	/*
	Private _cTopAlias := "TOTVSSPED"         //DEFINI?O DO SHadmin	EMA DE AMBIENTE ALIAS
	Private _cTopDB    := "MSSQL"       //BANCO DE DADOS UTILIZADO
	Private _cTopSrv   := "10.55.10.4" //= IP DO SERVIDOR
	Private _cTopSrvN  := "10.55.10.4"
	Private cTopServer
	Private cTopAlias

	cTop         := GetPvProfString("TOTVSSPED","TOPALIAS",_cTopAlias,GetAdv97())
	cTopData     := GetPvProfString("MSSQL","TOPDATABASE",_cTopDB,GetAdv97())
	cTopAlias    := cTopData + "/" + cTop
	cTopServer   := GetPvProfString("10.55.10.4","TOPSERVER",_cTopSrv,GetAdv97())
...
///
	...
	*/
	...
	oWS:cChvInicial	:= cChvNfe //cChvIni
	oWS:cChvFinal	:= cChvNfe //cChvFin
...
If !Empty(cXml)	
	...
	MMEMO1     := cXml//"<xCorrecao>"//TMP->TMEMO1     ///Relativo ao envio
	MMEMO2     := cXml //TMP->TMEMO2     ///Retorno da SEFAZ
	MNFE_CHV   := cChvNfe //TMP->NFE_CHV
	MID_EVENTO := oXml:_PROCEVENTONFE:_EVENTO:_INFEVENTO:_ID:TEXT//TMP->ID_EVENTO
	MTPEVENTO  := oXml:_PROCEVENTONFE:_EVENTO:_INFEVENTO:_TPEVENTO:TEXT//STR(TMP->TPEVENTO,6)
	MSEQEVENTO := oXml:_PROCEVENTONFE:_EVENTO:_INFEVENTO:_NSEQEVENTO:TEXT//STR(TMP->SEQEVENTO,1)
	MAMBIENTE  := "Produção"//STR(TMP->AMBIENTE,1)+IIF(TMP->AMBIENTE==1," - Produção", IIF(TMP->AMBIENTE==2," - Homologação" , ""))
	MDATE_EVEN := SUBSTR(oXml:_PROCEVENTONFE:_EVENTO:_INFEVENTO:_DHEVENTO:TEXT,9,2)+"/"+SUBSTR(oXml:_PROCEVENTONFE:_EVENTO:_INFEVENTO:_DHEVENTO:TEXT,6,2)+"/"+SUBSTR(oXml:_PROCEVENTONFE:_EVENTO:_INFEVENTO:_DHEVENTO:TEXT,1,4)
	MTIME_EVEN := SUBSTR(oXml:_PROCEVENTONFE:_EVENTO:_INFEVENTO:_DHEVENTO:TEXT,12,8)//TMP->TIME_EVEN
	MVERSAO    := oXml:_PROCEVENTONFE:_EVENTO:_VERSAO:TEXT
	MVEREVENTO := oXml:_PROCEVENTONFE:_EVENTO:_INFEVENTO:_VEREVENTO:TEXT
	MVERTPEVEN := oXml:_PROCEVENTONFE:_EVENTO:_VERSAO:TEXT
	MCORGAO    := oXml:_PROCEVENTONFE:_EVENTO:_INFEVENTO:_CORGAO:TEXT//STR(TMP->CORGAO,2)+IIF(TMP->CORGAO==13 , " - AMAZONAS",IIF(TMP->CORGAO==35 , " - SAO PAULO" , ""))
	MCSTATEVEN := ""//STR(TMP->CSTATEVEN,3)
	MCMOTEVEN  := ""//TMP->CMOTEVEN
	//STR(TMP->PROTOCOLO,15)
...
//	xEnder  := RTRIM(SM0->M0_ENDENT) + " - " + RTRIM(SM0->M0_BAIRENT) + " - " + RTRIM(SM0->M0_CIDENT) + "/" + SM0->M0_ESTENT
	xEnder  := RTRIM(SM0->M0_ENDENT) 
	xEndere := " - " + RTRIM(SM0->M0_BAIRENT) + " - " + RTRIM(SM0->M0_CIDENT) + "/" + SM0->M0_ESTENT
...
////
	...
	MVERAPLIC  := ""
	iw1 := AT("<verAplic>" , MMEMO1 )
	iw2 := AT("</verAplic>" , MMEMO1 )
	IF ( iw1 > 0 )
		iw3 := ( iw2 - iw1 )
		MVERAPLIC += SUBS(MMEMO1 , ( iw1+13 ) , ( iw2 - ( iw1 + 13 ) ) )
	ENDIF
	...
	IF ( iw1 > 0 )
		iw3 := ( iw2 - iw1 )
		MPROTOCOLO += SUBS(MMEMO1 , ( iw1+13 ) , ( iw2 - ( iw1 + 13 ) ) )
	ENDIF
	...
	IF ( iw1 > 0 )
		iw3 := ( iw2 - iw1 )
		MCSTATEVEN  += SUBS(MMEMO1 , ( iw1 + 7 ) , ( iw2 - ( iw1 + 7 ) ) )
	ENDIF
	
	MCMOTEVEN  := ""//TMP->CMOTEVEN

	iw1 := AT("<xMotivo>" , MMEMO1 )
	iw2 := AT("</xMotivo>" , MMEMO1 )
	IF ( iw1 > 0 )
		iw3 := ( iw2 - iw1 )
		MCMOTEVEN  += SUBS(MMEMO1 , ( iw1+9 ) , ( iw2 - ( iw1 + 9 ) ) )
	ENDIF
...
// Cria um novo objeto para impressao
	oPrint := TMSPrinter():New("Impresssão da Carta de Correção Eletronica - CC-e")
...
//oPrint:SayBitMap(100,116,xBitMap,600,280)
	oPrint:SayBitMap(100,116,xBitMap,400,200)

	oPrint:Say(100,780,xRazSoc,oFont11b ,140)
	oPrint:Say(160,780,xEnder,oFont11 ,140)
	oPrint:Say(210,780,xEndere,oFont11 ,140)
	oPrint:Say(260,780,xFone,oFont11 ,140)
	oPrint:Say(310,780,xCnpj,oFont11 ,140)
	oPrint:Say(350,780,xIE,oFont11 ,140)
*
...
//oPrint:Line(150,1890,150,2400)
	...
	oPrint:Say(104,2015,"Carta de Correção",oFont11b ,160)
...
//oPrint:Box(420,100,2000,2400)
	...
	oPrint:Say(440,850,"Data e hora de Impressão",oFont12b ,100)
	oPrint:Say(440,1890,"Protocolo",oFont12b ,100)
	oPrint:Say(490,110,"Carta de Correção NFe",oFont11 ,100)
	oPrint:Say(490,850,DTOC(dDataBase)+"  "+TIME(),oFont11 ,140)
